 ...
 * Module Dependencies
 */
var debug = require('debug')('nfirmata');
...
MIDI_RESPONSE[REPORT_VERSION] = function (buffer, board) {
...
MIDI_RESPONSE[ANALOG_MESSAGE] = function (buffer, board) {
...
MIDI_RESPONSE[DIGITAL_MESSAGE] = function (buffer, board) {
...
//SYSEX_RESPONSE[QUERY_FIRMWARE] = function (buffer, board) {
//  var firmwareBuf = [];
...
//SYSEX_RESPONSE[CAPABILITY_RESPONSE] = function (buffer, board) {
...
SYSEX_RESPONSE[PIN_STATE_RESPONSE] = function (buffer, board) {
...
SYSEX_RESPONSE[ANALOG_MAPPING_RESPONSE] = function (buffer, board) {
  var pin = 0;
  var currentValue;
...
SYSEX_RESPONSE[I2C_REPLY] = function (buffer, board) {
  var reply = [];
...
SYSEX_RESPONSE[ONEWIRE_DATA] = function (buffer, board) {
  ...
  SYSEX_RESPONSE[subCommand](buffer, board);
};

SYSEX_RESPONSE[ONEWIRE_SEARCH_REPLY] = function (buffer, board) {
...
SYSEX_RESPONSE[ONEWIRE_SEARCH_ALARMS_REPLY] = function (buffer, board) {
  var pin = board.currentBuffer[3];
  var replyBuffer = board.currentBuffer.slice(4, board.currentBuffer.length - 1);
...
SYSEX_RESPONSE[ONEWIRE_READ_REPLY] = function (buffer, board) {
...
SYSEX_RESPONSE[STRING_DATA] = function (buffer, board) {
...
SYSEX_RESPONSE[PING_READ] = function (buffer, board) {
 ...
 * @param {Function} [callback] A function to be called when the arduino is ready to communicate.
 */
function Board(options) {
  Emitter.call(this);

  if (typeof options === "string" || arguments.length > 1) {
    throw new Error('using connect');
  }
...
Board.prototype.defaultFeatures = function () {
  this.addFeature(require('./features/firmware')());
  this.addFeature(require('./features/capability')());
};

Board.prototype.addFeature = function (cmd, feature) {
  if (!_.isNumber(cmd)) {
    feature = cmd;
    cmd = null;
  }
  ...
  var layer = {
    cmd: cmd || feature.cmd || feature.command,
    feature: feature
  };
  this._features.push(layer);
};

Board.prototype.connect = function (port, opts, callback) {
  this.port = port;
  ...
  var that = this;
  if (!this.port) {
    Board.list(function (err, devices) {
      that.connect(devices[0], opts);
    });
    return this;
  }
  ...
  if (_.isObject(this.port) && typeof this.port.write === 'function') {
    this.transport = this.port;
  } else {
    this.transport = new SerialPort(this.port, opts.serialport);
  }
  ...
  this.transport.once('open', function () {
    debug('device opened');
    that.emit('connect'); // for test todo remove
    var cid = setInterval(function () {
      if (that.versionReceived === false) {
        debug('request REPORT_VERSION');
        that.reportVersion();
        debug('request  QUERY_FIRMWARE');
        that.queryFirmware();
      } else {
        clearInterval(cid);
      }
    ...
    that.once('reportversion', function () {
      that.versionReceived = true;
      that.once("queryfirmware", function () {
        clearInterval(cid);
        ...
        if (opts.samplingInterval !== undefined) {
          that.setSamplingInterval(opts.samplingInterval);
        }
        if (opts.skipCapabilities) {
          that.analogPins = opts.analogPins || that.analogPins;
          that.pins = opts.pins || that.pins;
          if (!that.pins.length) {
            for (var i = 0; i < (opts.pinCount || MAX_PIN_COUNT); i++) {
              var analogChannel = that.analogPins.indexOf(i);
              if (analogChannel < 0) {
                analogChannel = 127;
              }
              that.pins.push({supportedModes: [], analogChannel: analogChannel});
            }
          }
          connected();
        } else {
          that.queryCapabilities(function () {
            that.queryAnalogMapping(connected);
          });
        }
      });
    });

    that.transport.on('data', function (data) {
      that._handleData(data);
    })
  });
};

Board.prototype._handleWithFeatures = function (cmd, data) {
  for (var i = 0; i < this._features.length; i++) {
    var layer = this._features[i];
    if (layer.cmd && layer.cmd === cmd) {
      layer.feature.handle(cmd, data, this);
      return true;
    } else if (layer.feature.handle(cmd, data, this)) {
      return true;
    }
  }
  return false;
...
Board.prototype._handleData = function (data) {
  var byt, cmd, buf;
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (this.currentBuffer[0] === START_SYSEX &&
      ...
      cmd = this.currentBuffer[1];
      buf = this.currentBuffer.slice(2, this.currentBuffer.length - 1);

      if (SYSEX_RESPONSE[cmd]) {
        SYSEX_RESPONSE[cmd](buf, this);
      } else {
        this._handleWithFeatures(cmd, buf, this);
        this.emit("sysex", cmd, buf, this);
      }
    ...
    if (this.currentBuffer.length === 3 && this.currentBuffer[0] !== START_SYSEX) {
      ...
      if (MIDI_RESPONSE[cmd]) {
        MIDI_RESPONSE[cmd](this.currentBuffer, this);
        this.currentBuffer.length = 0;
      } else {
...
Object.defineProperty(Board.prototype, 'isReady', {
  get: function () {
...
Board.prototype.send = function (cmd, data, cb) {
  //data = flatten(Array.prototype.slice.call(arguments, 1));
  var buf = [];
  buf.push(START_SYSEX);
  buf.push(cmd);
  buf = buf.concat(Encoder7Bit.to7BitArray(data));
  buf.push(END_SYSEX);
  console.log('[Firmata] sent', data.length, Buffer.isBuffer(data) ? data : new Buffer(data));
  this.transport.write(buf, cb);
};
...
Board.prototype.reportVersion = function (callback) {
  if (callback) this.once("reportversion", callback);
...
Board.prototype.queryFirmware = function (callback) {
  if (callback) this.once("queryfirmware", callback);
...
Board.prototype.analogRead = function (pin, callback) {
  ...
  if (callback) this.addListener("analog-read-" + pin, callback);
};
...
Board.prototype.reset = function (cb) {
  this.transport.write(new Buffer([SYSTEM_RESET]), cb);
};

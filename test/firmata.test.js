...
describe("board", function () {
  ...
  var board = new Board();
  board.connect(serialPort, function (err) {
    boardStarted = true;
  ...
  it("uses serialport defaults", function (done) {
    var a = new Board();
    a.connect("/path/to/fake/usb1", function (err) {
    });
    var b = new Board();
    b.connect("/path/to/fake/usb2", function (err) {
    });

    should.equal(spy.getCall(0).args[0], "/path/to/fake/usb1");
    should.deepEqual(spy.getCall(0).args[1], {baudrate: 115200, buffersize: 1});

    should.equal(spy.getCall(1).args[0], "/path/to/fake/usb2");
    should.deepEqual(spy.getCall(1).args[1], {baudrate: 115200, buffersize: 1});
  ...
  it("emits 'ready' after handshakes complete (skipCapabilities)", function (done) {
    var serialPort = new SerialPort("/path/to/fake/usb");
    var board = new Board();
    board.connect(serialPort, {skipCapabilities: true}, function (err) {
    });
    var connected = false;
  ...
  it("sends report version and query firmware if it hasnt received the version within the timeout", function (done) {
    ...
    var board = new Board();
    board.connect(serialPort, opt, function (err) {
    });
  ...
  it("uses default baud rate and buffer size", function (done) {
    ...
    should.deepEqual(
      spy.args, [["fake port", {baudrate: 115200, buffersize: 1}]]
    );
  ...
  it("overrides baud rate and buffer size", function (done) {
    ...
    var board = new Board();
    board.connect(port, opt, function (err) {
    });
  ...
  it("Optionally call setSamplingInterval after queryfirmware", function (done) {
    ...
    var board = new Board();
    board.connect(serialPort, options, function (err) {
  ...
  it("Does not call setSamplingInterval after queryfirmware by default", function (done) {
    ...
    var board = new Board();
    board.connect(serialPort, options, function () {
      should.equal(spy.callCount, 1);
      spy.restore();
      done();
    });
  ...
  it("should be able to read value of analog pin on a board that skipped capabilities check", function (done) {
    var serialPort = new SerialPort("/path/to/fake/usb");
    var board = new Board();
    board.connect(serialPort, {skipCapabilities: true, analogPins: [14, 15, 16, 17, 18, 19]}, function (err) {
    });
